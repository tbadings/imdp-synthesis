...
class Drone6D(DroneDynamics):
    ...
    def set_spec(self):
        ...
        self.pi_arch = [64, 64, 64]
        self.vf_arch = [64, 64, 64]
...
class Drone6D_medium(DroneDynamics):
    '''
    Drone benchmark, with a 6D state space and a 3D control input space.
    '''
    ...
    def set_spec(self):
        ...
        self.goal = np.array([
            [[3, v_min, -2, v_min, -7, v_min], [7, v_max, 2, v_max, 7, v_max]]
        ], dtype=float)

        self.critical = np.array([
            [[-2, v_min, -3, v_min, -7, v_min], [1, v_max, 3, v_max, 7, v_max]],
        ], dtype=float)

        self.x0 = np.array([-5.5, 0.01, 0, 0.01, 0.01, 0.01])
