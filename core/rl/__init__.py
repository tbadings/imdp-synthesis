...
def find_active(model, args):
    ...
    rl_device = getattr(args, "rl_device_obj", None)
    if rl_device is None:
        try:
            rl_device = jax.devices("cpu")[0]
        except Exception:
            rl_device = None
    ...
    if rl_device is not None:
        logger.info("Running RL pipeline on device: %s", rl_device)
        with jax.default_device(rl_device):
            return _run()
    return _run()
