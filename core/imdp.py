...
def robust_value_iteration_fast(imdp, s0=None, max_iterations=1000, epsilon=1e-6):
    """
    Robust value iteration for interval MDPs.
    ...
    P_id_plusAbs = deepcopy(imdp.P_id)
    P_full_plusAbs = deepcopy(imdp.P_full)
    ...
    for iteration in pbar:
        ...
        for s in imdp.states:
            ...
            for a_idx, (a_label, successors) in enumerate(imdp.P_id[s].items()):
                ...
                extra_probs = np.minimum(prob_ub[sort] - prob_lb[sort], budget)
                extra_probs = np.minimum(extra_probs, np.maximum(0, budget - np.cumsum(extra_probs) + extra_probs))
                probs = prob_lb[sort] + extra_probs
